modifier_forest_fade = class ({})

function modifier_forest_fade:GetTexture()
    return "generic/forest_fade"
end

function modifier_forest_fade:IsHidden() return false end
function modifier_forest_fade:IsPurgable() return false end

function modifier_forest_fade:DeclareFunctions()
    return {MODIFIER_PROPERTY_INVISIBILITY_LEVEL,
            MODIFIER_EVENT_ON_ATTACK,
            MODIFIER_EVENT_ON_ABILITY_FULLY_CAST}
end
    
function modifier_forest_fade:GetModifierInvisibilityLevel()
...
function modifier_forest_fade:CheckState()
    if IsServer() then
        if self:GetStackCount() == 0 then
...
function modifier_forest_fade:OnCreated(kv)
    ...
    if kv.isCosmetic then
        ...
        if self:GetCaster():GetOwnerEntity():FindModifierByName(self:GetName()) then
            ...
            self.cooldown = self:GetCaster():GetOwnerEntity():FindModifierByName("modifier_forest_fade"):GetStackCount()
        end
    else
...
function modifier_forest_fade:OnAttack(keys)
    if not IsServer() then return end
...
-- break on spell cast
function modifier_forest_fade:OnAbilityFullyCast(keys)
    if not IsServer() then return end
...
function modifier_forest_fade:OnIntervalThink()
    if not IsServer() then return end
